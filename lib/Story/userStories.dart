...
// Fetch data from Firestore for a particular user
Future<List<Map<String, dynamic>>> fetchStories(String userId) async {
  // Query Firestore to get documents where the userId matches the provided userId
  final QuerySnapshot snapshot = await FirebaseFirestore.instance
      .collection('uploads')
      .where('userId', isEqualTo: userId) // Filter by specific user's ID
...
class Userstories extends StatefulWidget {
  ...
  const Userstories({super.key, required this.userId});
  @override
  State createState() => _StoriesPageViewState();
}


class _StoriesPageViewState extends State<Userstories> {
  ...
  void initState() {
    super.initState();
    _storiesFuture = fetchStories(widget.userId);
